...
def validate_foundation_summary(summary: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(corpus, dict) or corpus.get("sources", 0) != 74 or corpus.get("documents", 0) != 116 or corpus.get("chunks", 0) <= 0:
...
def _run_corpus_and_ingestion(temp_root: Path) -> dict[str, Any]:
    ...
    checked_in_root = Path("demo/trade_intel_seed").resolve()
    ...
    import yaml
    payload = yaml.safe_load(Path("data/sources/trade_intel_demo.yaml").read_text(encoding="utf-8"))
...
def run_foundation_smoke(*, settings: Settings | None = None) -> dict[str, Any]:
    ...
    corpus_state = _stage("corpus", lambda: _run_corpus_and_ingestion(Path(tempfile.mkdtemp(prefix="trade-foundation-smoke-"))))
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify the synthetic trade-agent foundation.")
    parser.parse_args()
    try:
